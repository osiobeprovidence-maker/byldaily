...
export function Articles() {
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    else { searchParams.delete('q'); setSearchParams(searchParams); }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-16">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 border-b border-byl-dark/10 pb-8 gap-6">
        ...
        <div className="w-full md:w-auto flex-shrink-0 relative">
          ...
          <input type="text" className="block w-full md:w-72 pl-12 pr-4 py-3 border border-byl-dark/20 bg-byl-light placeholder-byl-dark/40 focus:outline-none focus:border-byl-purple text-[11px] uppercase font-bold tracking-widest transition-colors duration-300 text-byl-dark" placeholder="Search stories..." value={searchQuery} onChange={handleSearchChange} />
        </div>
      </div>
      ...
      <div className="flex overflow-x-auto pb-4 mb-8 -mx-4 px-4 sm:mx-0 sm:px-0 hide-scrollbar space-x-4 border-b border-byl-dark/10">
        <button onClick={() => setActiveCategory('All')} className={`whitespace-nowrap px-2 pb-3 mb-[-1px] text-[11px] uppercase tracking-widest font-bold transition-colors ${activeCategory === 'All' ? 'text-byl-purple border-b border-byl-purple' : 'text-byl-dark/40 hover:text-byl-dark'}`}>All</button>
        {CATEGORIES.map(category => (
          <button key={category} onClick={() => setActiveCategory(category)} className={`whitespace-nowrap px-2 pb-3 mb-[-1px] text-[11px] uppercase tracking-widest font-bold transition-colors ${activeCategory === category ? 'text-byl-purple border-b border-byl-purple' : 'text-byl-dark/40 hover:text-byl-dark'}`}>{category}</button>
        ))}
      </div>

      {!articles ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 animate-pulse">
          {[1,2,3,4,5,6].map(i => <div key={i} className="h-96 bg-byl-dark/5" />)}
        </div>
              ...
              <Link key={article._id} to={`/articles/${article._id}`} className="group flex flex-col items-start bg-byl-light border border-byl-dark/10 hover:border-byl-dark transition-colors duration-300">
                <div className="w-full h-56 overflow-hidden relative border-b border-byl-dark/10">
                  ...
                  <div className="absolute top-0 left-0">
                    <span className="px-4 py-2 bg-byl-dark text-byl-light text-[10px] font-bold uppercase tracking-widest transition-colors">{article.category}</span>
                  </div>
                </div>
