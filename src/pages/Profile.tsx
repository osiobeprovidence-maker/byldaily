...
import { Settings, LogOut, ShieldAlert, Sun, Moon, User as UserIcon } from 'lucide-react';
import { Link, useNavigate } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
import { useTheme } from '../context/ThemeContext';

export function Profile() {
  const [activeTab, setActiveTab] = React.useState<'Posts' | 'Events' | 'Forum' | 'Settings'>('Posts');
  ...
  const convexUser = allUsers?.find((u: any) => u.firebaseUid === user?.id);
  ...
  const myArticles = (articles ?? []).filter(a => a.authorId === convexUser?._id);
  const myForumPosts = (forumPosts ?? []).filter(p => p.authorId === user.id);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-12 md:py-16">
      <div className="bg-byl-light border border-byl-dark/10 p-8 md:p-12 mb-12 flex flex-col md:flex-row items-center md:items-start gap-8 transition-colors duration-300">
        ...
        <div className="flex-grow text-center md:text-left flex flex-col justify-center h-full">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            <div>
              <h1 className="font-serif text-3xl md:text-4xl font-bold text-byl-dark transition-colors">{user.name || "BYLDaily Member"}</h1>
              <p className="text-[11px] uppercase tracking-widest font-bold text-byl-dark/40 mt-1 transition-colors">@{user.username}</p>
            </div>
            <div className="flex flex-wrap gap-4">
              <button onClick={() => setActiveTab('Settings')} className="border border-byl-dark px-6 py-3 text-[11px] uppercase tracking-widest font-bold text-byl-dark hover:bg-black/5 transition-colors flex items-center justify-center space-x-2">
                <Settings size={14} /><span>Settings</span>
              </button>
            </div>
          </div>
          <p className="text-byl-dark/70 mb-6 font-medium leading-relaxed max-w-2xl text-sm md:text-base transition-colors">{user.bio || "A member of the BYLDaily community."}</p>
      ...
      <div className="flex overflow-x-auto pb-4 mb-10 -mx-4 px-4 sm:mx-0 sm:px-0 hide-scrollbar space-x-8 border-b border-byl-dark/10">
        {(['Posts', 'Events', 'Forum', 'Settings'] as const).map(tab => (
      ...
      <div className="min-h-[400px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {myArticles.length === 0 ? (
              ...
              myArticles.map(article => (
                <Link key={article._id} to={`/articles/${article._id}`} className="group flex flex-col items-start bg-byl-light border border-byl-dark/10 hover:border-byl-dark transition-colors duration-300">
                  <div className="w-full h-56 overflow-hidden relative border-b border-byl-dark/10">
                    ...
                    <div className="absolute top-0 left-0"><span className="px-4 py-2 bg-byl-dark text-byl-light text-[10px] font-bold uppercase tracking-widest transition-colors">{article.category}</span></div>
                  </div>
                  <div className="p-8 flex flex-col flex-grow w-full">
                    <h3 className="font-serif text-2xl font-bold text-byl-dark mb-4 group-hover:underline leading-tight transition-colors">{article.title}</h3>
                    <p className="text-byl-dark/60 text-sm line-clamp-3 mb-6 flex-grow font-medium transition-colors">{article.excerpt}</p>
                    <div className="flex items-center w-full mt-auto pt-6 border-t border-byl-dark/10 transition-colors">
                      <img src={convexUser?.avatarUrl || user.avatarUrl || "https://images.unsplash.com/photo-1522529599102-193c0d76b5b6?w=100&q=80"} alt={user.name} className="w-10 h-10 object-cover mr-4 grayscale" />
                      <div className="flex flex-col"><span className="text-[10px] uppercase tracking-widest font-bold text-byl-dark transition-colors">{convexUser?.name || user.name}</span><span className="text-[10px] uppercase font-bold text-byl-dark/40 transition-colors">{article.date}</span></div>
                    </div>
                  </div>
                </Link>
              ))
        ...
        {activeTab === 'Events' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {(events ?? []).length === 0 ? (
              <p className="col-span-full text-byl-dark/50 text-center font-medium mt-10 transition-colors">No events available right now.</p>
            ) : (
              (events ?? []).map(event => (
                <div key={event._id} className="flex flex-col sm:flex-row bg-byl-light border border-byl-dark/10 overflow-hidden hover:border-byl-dark transition-colors group">
                  ...
                  <div className="p-6 flex flex-col justify-center">
                    ...
                    <div className="flex items-center text-[10px] uppercase tracking-widest font-bold text-byl-dark/60 mb-2 transition-colors">{event.date}</div>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="flex flex-col space-y-6 max-w-3xl">
            {myForumPosts.length === 0 ? (
              ...
              myForumPosts.map(post => (
                <div key={post._id} className="bg-byl-light border border-byl-dark/10 p-6 hover:border-byl-dark transition-colors duration-300">
                  <span className="text-[10px] uppercase font-bold tracking-widest text-byl-purple mb-2 block">{post.topic}</span>
                  <p className="text-byl-dark font-medium leading-relaxed mb-4 transition-colors">{post.content}</p>
          ...
          <div className="max-w-xl space-y-10">
            <section>
              ...
              <div className="flex items-center justify-between p-6 bg-byl-light border border-byl-dark/10">
                ...
                <button onClick={toggleTheme} className="px-6 py-3 bg-byl-dark text-byl-light text-[10px] uppercase font-bold tracking-widest hover:bg-byl-purple transition-all flex items-center space-x-2">
                  {theme === 'light' ? <Moon size={14} /> : <Sun size={14} />}<span>{theme === 'light' ? 'Dark' : 'Light'}</span>
                </button>
              </div>
            </section>
            ...
            {isAdmin && (
              <section>
                ...
                <div className="flex items-center justify-between p-6 bg-byl-dark text-byl-light">
                  ...
                  <Link to="/admin" className="px-6 py-3 bg-byl-purple text-white text-[10px] uppercase font-bold tracking-widest hover:bg-white hover:text-byl-purple transition-all flex items-center space-x-2">
                    <ShieldAlert size={14} /><span>Open Dashboard</span>
                  </Link>
                </div>
              </section>
            )}
            ...
            <section>
              ...
              <div className="space-y-4">
                <button className="w-full text-left p-6 bg-byl-light border border-byl-dark/10 flex items-center justify-between hover:border-byl-dark transition-colors group">
                  <div><p className="font-bold text-byl-dark">Edit Profile Information</p><p className="text-xs text-byl-dark/60">Update your name, bio, and interests.</p></div>
                  <Settings size={18} className="text-byl-dark/20 group-hover:text-byl-dark" />
                </button>
                <button onClick={handleLogout} className="w-full text-left p-6 bg-red-500/5 border border-red-500/10 flex items-center justify-between hover:bg-red-500/10 transition-colors group">
                  <div><p className="font-bold text-red-600">Logout Session</p><p className="text-xs text-red-600/60">Sign out of your current account.</p></div>
