...
import { Users, FileText, MessageSquare, BarChart3, Settings, TrendingUp, Plus, MoreHorizontal, Search, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        ...
        <p className="text-byl-dark/60 max-w-md mx-auto">You do not have administrative privileges to access this section. Please contact the system administrator if you believe this is an error.</p>
        <Link to="/" className="mt-8 bg-byl-dark text-byl-light px-8 py-4 text-[11px] uppercase tracking-widest font-bold hover:bg-byl-purple transition-all">Return Home</Link>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 border-b border-byl-dark/10 pb-8 gap-6">
        <div>
          ...
          <p className="text-lg text-byl-dark/60 font-medium">Platform management, insights, and moderation tools.</p>
        </div>
        <div className="flex items-center space-x-4">
          <div className="flex items-center text-[10px] uppercase font-bold tracking-widest text-green-600 bg-green-50 px-3 py-1 border border-green-200">
            <div className="w-2 h-2 bg-green-500 rounded-full mr-2 animate-pulse"></div>System Status: Live
          </div>
        </div>
      </div>
        ...
        <aside className="lg:w-1/4 shrink-0">
          <div className="flex flex-col space-y-2 sticky top-32">
            {[
              ...
              <button key={item.id} onClick={() => setActiveTab(item.id as any)} className={`flex items-center space-x-3 px-6 py-4 text-[11px] uppercase tracking-widest font-bold transition-all border ${activeTab === item.id ? 'bg-byl-dark text-byl-light border-byl-dark shadow-xl' : 'text-byl-dark/60 border-transparent hover:border-byl-dark/20 hover:bg-byl-dark/5'}`}>
...
function OverviewTab() {
  ...
  const stats = [
    { label: 'Articles', value: String(articles?.length ?? 0), trend: 'Published', color: 'text-byl-purple' },
    { label: 'Creators', value: String(creators?.length ?? 0), trend: 'Active', color: 'text-byl-dark' },
    { label: 'Discussions', value: String(forumPosts?.length ?? 0), trend: 'Active', color: 'text-byl-dark' },
    { label: 'Members', value: String(users?.length ?? 0), trend: 'Registered', color: 'text-green-600' },
  ];
        ...
        {stats.map((stat, i) => (
          <div key={i} className="bg-byl-light border border-byl-dark/10 p-8 transition-all hover:shadow-lg">
            ...
            <div className="flex items-end justify-between">
              ...
              <span className="text-[10px] font-bold text-green-600 flex items-center"><TrendingUp size={12} className="mr-1" />{stat.trend}</span>
            </div>
          </div>
        ))}
      </div>
      <div className="bg-byl-light border border-byl-dark/10 p-8">
        ...
        <div className="space-y-6">
          {(forumPosts ?? []).slice(0, 5).map((post: any) => (
            <div key={post._id} className="flex items-center justify-between py-2 border-b border-byl-dark/5 last:border-0">
              <div className="flex items-center space-x-4">
                <div className="w-8 h-8 flex items-center justify-center rounded-full bg-blue-100 text-blue-600"><MessageSquare size={14} /></div>
                <div>
                  <p className="text-sm font-bold text-byl-dark"><span className="mr-1">New discussion</span><span className="text-byl-dark/60 font-medium">in {post.topic}</span></p>
                  <p className="text-[10px] text-byl-dark/30 uppercase tracking-widest font-bold">{post.upvotes} upvotes</p>
                </div>
              </div>
              <button className="text-byl-dark/30 hover:text-byl-dark"><MoreHorizontal size={16} /></button>
            </div>
          ))}
          {(!forumPosts || forumPosts.length === 0) && (
            <p className="text-byl-dark/40 text-center font-medium py-4">No recent activity</p>
          )}
        </div>
      </div>
    </div>
...
function ContentTab({ articles, users: _users }: { articles: any[]; users: any[] }) {
  const userMap = new Map(_users.map((u: any) => [u._id, u]));
  return (
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-byl-dark/5">
            {articles.slice(0, 6).map((article: any) => {
              const author = userMap.get(article.authorId);
              return (
                <tr key={article._id} className="hover:bg-byl-dark/[0.01] transition-colors group">
                  ...
                  <td className="px-6 py-5 text-sm font-bold text-byl-dark/60">{author?.name}</td>
                  <td className="px-6 py-5"><span className="text-[10px] uppercase font-bold tracking-widest border border-byl-dark/10 px-2 py-1">{article.category}</span></td>
                  <td className="px-6 py-5"><span className="flex items-center text-[10px] uppercase font-bold tracking-widest text-green-600"><CheckCircle size={12} className="mr-1.5" /> Published</span></td>
                  <td className="px-6 py-5 text-right"><button className="text-byl-dark/30 hover:text-byl-dark transition-colors"><MoreHorizontal size={18} /></button></td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="bg-byl-light border border-byl-dark/10">
        ...
        <div className="divide-y divide-byl-dark/5">
          {creators.map((creator: any) => (
            <div key={creator._id} className="p-6 flex items-center justify-between group hover:bg-byl-dark/[0.01] transition-colors">
              ...
              <div className="flex items-center space-x-10 text-[10px] uppercase font-bold tracking-widest text-byl-dark/40">
                <div className="text-center"><p className="text-byl-dark">{(creator.followers/1000).toFixed(1)}k</p><p>Followers</p></div>
                ...
                <button className="text-byl-dark/30 hover:text-red-500 transition-colors"><XCircle size={18} /></button>
              </div>
            </div>
          ))}
        </div>
...
function CommunityTab({ forumPosts, users: _users }: { forumPosts: any[]; users: any[] }) {
  const userMap = new Map(_users.map((u: any) => [u._id, u]));
  const firebaseUserMap = new Map(_users.filter((u: any) => u.firebaseUid).map((u: any) => [u.firebaseUid, u]));
  return (
    <div className="space-y-8">
      <div className="flex items-center space-x-4 bg-orange-50 border border-orange-200 p-6">
        <AlertCircle className="text-orange-600" size={24} />
        <div>
          <p className="text-sm font-bold text-orange-900">Discussions require review</p>
          <p className="text-[11px] text-orange-700/70 font-medium italic">Review posts for community guidelines.</p>
        </div>
      </div>
      <div className="bg-byl-light border border-byl-dark/10 overflow-hidden">
        <div className="p-8 border-b border-byl-dark/5"><h3 className="text-[11px] uppercase tracking-widest font-bold">Recent Discussions</h3></div>
        <div className="divide-y divide-byl-dark/5">
          {forumPosts.slice(0, 5).map((post: any) => {
            const author = firebaseUserMap.get(post.authorId) ?? userMap.get(post.authorId);
            return (
              <div key={post._id} className="p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-byl-dark/[0.01]">
                <div className="flex-grow max-w-2xl">
                  <div className="flex items-center space-x-2 mb-3">
                    <span className="text-[10px] uppercase font-bold tracking-widest px-2 py-0.5 bg-byl-purple text-white">{post.topic}</span>
                    <span className="text-[10px] font-bold text-byl-dark/30 uppercase tracking-widest">by {author?.name}</span>
                  </div>
                  <p className="text-byl-dark font-medium line-clamp-2 leading-relaxed">{post.content}</p>
                </div>
                <div className="flex items-center space-x-4 shrink-0">
                  <button className="flex items-center space-x-1.5 text-green-600 border border-green-200 px-4 py-2 text-[10px] uppercase tracking-widest font-bold hover:bg-green-600 hover:text-white transition-all"><CheckCircle size={14} /><span>Keep</span></button>
                  <button className="flex items-center space-x-1.5 text-red-600 border border-red-200 px-4 py-2 text-[10px] uppercase tracking-widest font-bold hover:bg-red-600 hover:text-white transition-all"><XCircle size={14} /><span>Remove</span></button>
                </div>
              </div>
            );
          })}
        ...
        <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-byl-light border border-byl-dark/10 p-10 shadow-xl">
          <div className="space-y-6">
            {['instagram', 'twitter', 'facebook'].map(platform => (
              <div key={platform}>
                <label className="block text-[10px] uppercase tracking-widest font-bold text-byl-dark/40 mb-2">{platform.charAt(0).toUpperCase() + platform.slice(1)} URL</label>
                <input type="url" value={(socialLinks as any)[platform]} onChange={(e) => setSocialLinks({...socialLinks, [platform]: e.target.value})} className="w-full bg-byl-dark/5 border-none px-4 py-3 text-sm font-medium focus:ring-1 focus:ring-byl-purple transition-all outline-none text-byl-dark" />
              </div>
            ))}
          </div>
          <div className="space-y-6">
            ...
            <div className="pt-6">
              <button type="submit" className="w-full bg-byl-dark text-byl-light py-4 text-[11px] uppercase tracking-widest font-bold hover:bg-byl-purple transition-all shadow-lg">Save Changes</button>
            </div>
          </div>
        </form>
      </section>
      <section>
        <div className="mb-8 border-b border-byl-dark/10 pb-4"><h3 className="text-[11px] uppercase tracking-widest font-black text-byl-dark">Platform Maintenance</h3></div>
        <div className="bg-byl-light border border-byl-dark/10 p-10 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <button className="bg-byl-dark/5 border border-byl-dark/10 text-byl-dark px-8 py-3 text-[10px] uppercase tracking-widest font-bold hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-all">Activate</button>
        </div>
      </section>
    </div>
  );
