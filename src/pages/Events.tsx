...
export function Events() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 lg:px-10 py-16">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-20 border-b border-byl-dark/10 pb-12">
        <div className="flex items-center space-x-3 mb-6">
          <div className="w-10 h-10 rounded-full bg-byl-purple/10 flex items-center justify-center text-byl-purple animate-pulse"><Star size={20} fill="currentColor" /></div>
          <span className="text-[10px] uppercase tracking-[0.3em] font-black text-byl-dark/40">Exclusive Programming</span>
        </div>
        <h1 className="font-serif text-6xl md:text-8xl font-black tracking-tighter leading-none mb-8 text-byl-dark">BYL Live.</h1>
        <p className="text-xl md:text-2xl text-byl-dark/60 max-w-3xl font-medium leading-relaxed italic">Experience the culture in real-time. From digital workshops to physical galleries, we're curating the moments that matter.</p>
      </motion.div>

      {featuredEvent && (
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="mb-24 group relative overflow-hidden bg-byl-dark text-byl-light flex flex-col lg:flex-row shadow-2xl items-stretch">
          <div className="lg:w-3/5 min-h-[400px] lg:h-auto overflow-hidden relative bg-black">
            <img src={featuredEvent.imageUrl} alt={featuredEvent.title} referrerPolicy="no-referrer" className="absolute inset-0 w-full h-full object-cover grayscale-[10%] opacity-90 group-hover:grayscale-0 group-hover:scale-105 group-hover:opacity-100 transition-all duration-1000 ease-out" />
          ...
          <div className="lg:w-2/5 p-12 lg:p-16 flex flex-col justify-center relative z-10 bg-byl-dark text-byl-light">
            ...
            <h2 className="font-serif text-4xl lg:text-5xl font-black mb-8 leading-tight tracking-tight">{featuredEvent.title}</h2>
          ...
          <div className="border-b border-byl-dark/10 md:border-none pb-6 md:pb-0">
            ...
            <div className="flex md:flex-col items-center md:items-start overflow-x-auto md:overflow-x-visible pb-2 md:pb-0 gap-8 md:gap-0 md:space-y-4 hide-scrollbar -mx-6 px-6 md:mx-0 md:px-0">
              {CATEGORIES.map(category => (
                <button key={category} onClick={() => setActiveCategory(category)} className={`whitespace-nowrap text-[11px] uppercase tracking-widest font-bold transition-all shrink-0 ${activeCategory === category ? 'text-byl-purple md:translate-x-2 border-b-2 border-byl-purple md:border-none pb-2 md:pb-0' : 'text-byl-dark/40 hover:text-byl-dark md:hover:translate-x-1'}`}>{category === 'All' ? 'Every Occasion' : category}</button>
              ))}
            </div>
          </div>
                ...
                {filteredEvents.map(event => (
                  <motion.div layout key={event._id} className="group flex flex-col bg-byl-light border border-byl-dark/10 hover:border-byl-dark transition-all duration-500 shadow-sm hover:shadow-2xl">
                    <div className="w-full h-64 md:h-72 overflow-hidden relative border-b border-byl-dark/10">
                      <img src={event.imageUrl} alt={event.title} referrerPolicy="no-referrer" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 grayscale group-hover:grayscale-0" />
                      <div className="absolute top-6 left-6">
                        <span className="px-4 py-2 bg-byl-light text-byl-dark text-[9px] font-black uppercase tracking-widest shadow-xl border border-byl-dark/10">{event.category}</span>
                      </div>
                    </div>
