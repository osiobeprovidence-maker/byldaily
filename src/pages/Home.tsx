        ...
        <aside className="lg:col-span-4 bg-byl-dark/[0.02] p-6 sm:p-10 lg:p-16 flex flex-col transition-colors duration-300">
          ...
          <div className="flex flex-col gap-10">
            {trendingArticles.map((article, idx) => (
              <Link key={article._id} to={`/articles/${article._id}`} className="group flex gap-6 cursor-pointer items-start">
            ...
            {!articles && (
              <div className="animate-pulse space-y-6">
                {[1,2,3].map(i => <div key={i} className="h-16 bg-byl-dark/5" />)}
              </div>
            )}
          </div>
        </aside>
      ...
      {featuredArticle && (
        <section className="px-4 sm:px-6 lg:px-10 mt-20">
          <div className="max-w-7xl mx-auto">
            ...
            <Link to={`/articles/${featuredArticle._id}`} className="group block relative overflow-hidden cursor-pointer border border-byl-dark/10 hover:border-byl-purple transition-colors">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
            {(allCreators ?? []).slice(0, 4).map(creator => (
              <div key={creator._id} className="flex flex-col items-start p-8 bg-byl-light border border-byl-dark/10 hover:border-byl-purple transition-colors group">
            ...
            {!allCreators && (
              <div className="animate-pulse grid grid-cols-4 gap-10 col-span-full">
                {[1,2,3,4].map(i => <div key={i} className="h-64 bg-byl-dark/5" />)}
              </div>
            )}
          </div>
        </div>
